  ...
  return (
    <SidebarContext value={contextValue}>
      <div
        ...
        className={cn(
          'group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex h-full w-full max-w-full overflow-x-hidden',
          className,
        )}
        {...props}
      >
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  const handlePointerDown = React.useCallback((e: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (wrapper && sidebarGap) {
      const ribbonWidth = Number.parseInt(getComputedStyle(wrapper).getPropertyValue('--sidebar-ribbon-width') || '0', 10);
  ...
  return (
    <button
      ...
      className={cn(
        'hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex select-none',
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'scrollbar-custom gap-0 flex min-h-0 flex-1 flex-col overflow-y-auto overflow-x-hidden group-data-[collapsible=icon]:overflow-hidden',
        className,
      )}
      {...props}
    />
